...
if clientID != -1:
    ...
    # Goal position (x, y)
    list_pgoal = [np.array([4, 0]), np.array([4, -4]), np.array([0, 0])]
    # pgoal = np.array([4, 0])
    # pgoal = np.array([0, 0])

    for pgoal in list_pgoal:
        ...
        returnCode = sim.simxSetObjectPosition(
            clientID, goalFrame, -1, [pgoal[0], pgoal[1], 0], sim.simx_opmode_oneshot_wait)
        returnCode = sim.simxSetObjectOrientation(
            clientID, goalFrame, -1, [0, 0, 0], sim.simx_opmode_oneshot_wait)
        ...
        while rho > .05:
            ...
            sim.simxSetJointTargetVelocity(
                clientID, robotRightMotorHandle, vr, sim.simx_opmode_oneshot_wait)
            sim.simxSetJointTargetVelocity(
                clientID, robotLeftMotorHandle, vl, sim.simx_opmode_oneshot_wait)
